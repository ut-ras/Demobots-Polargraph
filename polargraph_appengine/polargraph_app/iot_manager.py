...
# Originally from cloud python api example https://github.com/GoogleCloudPlatform/python-docs-samples/blob/master/iot/api-client/manager/manager.py
# REST API reference here: https://cloud.google.com/iot/docs/reference/cloudiot/rest/
# Modifications from Original:
#   organized getConfigVersions and getState into getState, getConfig, getConfigVersions, and getStateVersions
...
def str_from_binary_data(binaryData):
    data = str(base64.b64decode(binaryData))
    data = data[2:len(data) - 1]
    if data[len(data) - 2] == ',':
        data = data[0:len(data) - 2] + "}"
    #print("binary data string " + data)
    return data

def dict_from_str(json):
    try:
        data = ast.literal_eval(str(json))
    except (ValueError, SyntaxError):
        data = {}
    return data

def dict_from_binary_data(binaryData):
    data = str_from_binary_data(binaryData)
    return dict_from_str(data)
...
def get_state(
    ...
    device = get_device(service_account_json, project_id, cloud_region, registry_id, device_id)
    print(device)
    state = device.get('state', [])
    state['data'] = dict_from_binary_data(state['binaryData'])

    print('IOT DEVICE STATE \n\tbinaryData: {}\n'.format(state.get('binaryData')))
...
def get_state_versions(
    ...
    device_name = '{}/devices/{}'.format(registry_name, device_id)
    devices = client.projects().locations().registries().devices()
    states = devices.states().list(name=device_name, numStates=5).execute().get('deviceStates', [])

    for state in states:
        state['data'] = dict_from_binary_data(state['binaryData'])

    print('IOT DEVICE STATE \n\tbinaryData: {}\n'.format(states[0].get('binaryData')))
...
def get_config(
    ...
    config['data'] = dict_from_binary_data(config['binaryData'])

    print('IOT DEVICE CONFIG \n\tversion: {}\n\tcloudUpdateTime: {}\n\t binaryData: {}'.format(
        config.get('version'),
